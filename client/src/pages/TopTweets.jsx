...
function TopTweets() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('viewTweets', handleViewTweets);
    window.addEventListener('fetchTweets', handleFetchTweets);

    return () => {
      window.removeEventListener('viewTweets', handleViewTweets);
      window.removeEventListener('fetchTweets', handleFetchTweets);
    };
  }, []);
  ...
  const handleFetchTweetsAction = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/scrape_top_tweets`, {
        method: 'POST',
      ...
      if (response.ok) {
        const data = await response.json()
        console.log('Fetch tweets complete:', data)
        updateFetchComplete(true)
        // After fetching, automatically refresh the view
        handleViewTweetsAction()
      } else {
        const errorData = await response.json()
        setError(errorData.detail || 'Failed to fetch tweets')
      }
    } catch (err) {
  ...
  const handleViewTweetsAction = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/fetch_pending_replies_toptweets`, {
        method: 'GET',
        credentials: 'include'
      })
  ...
  const handleReplyAction = async (tweetId, action, editedText = null) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/handle_reply_action_toptweets`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const TwitterPost = ({ tweet, isReply = false }) => {
    const getInitials = (username) => {
      return username ? username.charAt(0).toUpperCase() : 'T'
    }

    return (
      <div className={`border-b border-gray-200 p-4 hover:bg-gray-50 transition-colors max-md:p-3 max-md:overflow-x-hidden max-md:w-full ${isReply ? 'bg-gray-50 border-l-4 border-blue-200 ml-4 max-md:ml-2' : ''}`}>
        <div className="flex space-x-3 max-md:space-x-2 max-md:overflow-x-hidden max-md:w-full">
          ...
          <div className="w-12 h-12 rounded-full overflow-hidden bg-blue-500 flex items-center justify-center flex-shrink-0 max-md:w-10 max-md:h-10">
            {/* Profile image */}
            {!isReply && tweet.profile_image_url && (
              <img 
            ...
            <span 
              ...
              style={{
                display: (!isReply && tweet.profile_image_url) || (isReply && userProfileImage) ? 'none' : 'flex'
              }}
            >
              {getInitials(isReply ? 'You' : tweet.username)}
            </span>
          </div>
          ...
          <div className="flex-1 min-w-0 max-md:overflow-x-hidden max-md:w-full">
            ...
            <div className="flex items-center space-x-2 mb-1 max-md:space-x-1 max-md:overflow-x-hidden">
              <h4 className="font-bold text-gray-900 truncate max-md:text-base max-md:overflow-x-hidden">
                {isReply ? 'You' : tweet.username}
              </h4>
            </div>
            ...
            <div className="max-md:w-full max-md:overflow-x-hidden">
              <p className="text-gray-900 mb-3 leading-relaxed max-md:mb-2 max-md:text-base max-md:break-words max-md:word-wrap max-md:overflow-wrap-anywhere max-md:w-full">
                {isReply ? (editingTweet === tweet.tweet_id ? editText : tweet.draft_reply) : tweet.text}
              </p>
            </div>

            {/* Edit textarea for reply */}
            {isReply && editingTweet === tweet.tweet_id && (
              <div className="mb-3 max-md:w-full max-md:overflow-x-hidden">
                <textarea
                  ...
                  rows="3"
                />
                <div className="flex space-x-2 mt-2 max-md:flex-wrap max-md:gap-2">
                  <button
            ...
            {/* Reply Actions for the drafted reply */}
            {isReply && editingTweet !== tweet.tweet_id && (
        ...
        <div className="max-md:overflow-x-hidden">
          <div className="p-4 border-b border-gray-200 max-md:p-3">
            <h3 className="text-lg font-semibold text-gray-900 max-md:text-lg">
              Tweets with Drafted Replies ({tweets.length})
            </h3>
          </div>
          ...
          {tweets.length === 0 ? (
            ...
            <div className="max-md:overflow-x-hidden">
              {tweets.map((tweet) => (
                <div key={tweet.tweet_id} className="max-md:overflow-x-hidden max-md:w-full">
                  {/* Original Tweet */}
                  <TwitterPost tweet={tweet} isReply={false} />
                  {/* Drafted Reply */}
                  <TwitterPost tweet={tweet} isReply={true} />
                </div>
              ))}
            </div>
          )}
